// Adapted from https://github.com/gparlakov/forms-typed/blob/master/projects/forms/src/lib/forms-typed.ts
import {
  AbstractControl,
...
export type Controls<K> =
  ...
  | {
      [k in keyof K]: K[k] extends Array<infer T>
        ?
            | TypedFormControl<K[k]>
            | TypedFormGroup<K[k]>
            | TypedFormArray<K[k], T>
 ...
 */
export type TypedArraysIn<K, arrays extends keyof K> = TypedControlsIn<
  K,
  never,
  arrays
>;
 ...
 */
export type TypedGroupsIn<K, groups extends keyof K> = TypedControlsIn<
  K,
  groups,
  never
>;
